...
-- WRONG (fail-open - allows all rows if context unset)
...
**CRITICAL:** Always use the fail-closed pattern shown above. Never use `COALESCE` with a fallback that makes the policy always true.
...
### Cross-tenant data leak (SECURITY)
...
This ensures queries return no rows when context is unset, preventing accidental data leaks.
